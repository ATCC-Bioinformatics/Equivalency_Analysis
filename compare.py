...
pub_length = df_pub_comp_mean[['ATCC Catalog', 'Total Length']]
port_length.columns = ['Catalog ID', 'Total Length']
pub_length.columns = ['Catalog ID', 'Total Length'] #need same column names to merge dataframes
...
df_len.to_csv('Assembly_Complete_length_comparisons.txt',sep='\t',index=False)

#N50 #compared by max N50
port_N50= df_port_complete[['Base Catalog Number', 'Filtered N50']]
pub_N50 = df_pub_comp_mean[['ATCC Catalog', 'Scaffold N50']]
port_N50.columns = ['Catalog ID', 'N50']
pub_N50.columns = ['Catalog ID', 'N50']
...
df_N50.to_csv('Assembly_Complete_N50_comparisons.txt',sep='\t',index=False)

#contig/replicon count #compared by min
port_contig= df_port_complete[['Base Catalog Number', 'Total Contigs']]
pub_contig = df_pub_comp_min[['ATCC Catalog', 'Scaffold Count']]
port_contig.columns = ['Catalog ID', 'Contig Count']
pub_contig.columns = ['Catalog ID', 'Contig Count']
...
df_contig.to_csv('Assembly_Complete_Contig_comparisons.txt',sep='\t',index=False)

#GC Content #compared by mean
port_gc= df_port_complete[['Base Catalog Number', 'GC Content']]
pub_gc = df_pub_comp_mean[['ATCC Catalog', 'GC Content']]
port_gc.columns = ['Catalog ID', 'GC']
port_gc['GC'] = port_gc['GC'] * 100
pub_gc.columns = ['Catalog ID', 'GC']
...
df_gc.to_csv('Assembly_Complete_GC_comparisons.txt',sep='\t',index=False)
...
pub_length = df_pub_scaffold_mean[['ATCC Catalog', 'Total Length']]
...
df_len.to_csv('Assembly_Scaffold_length_comparisons.txt',sep='\t',index=False)

#N50 #compared by max N50
#port_N50= df_port_complete[['Base Catalog Number', 'Filtered N50']]
pub_N50 = df_pub_scaffold_mean[['ATCC Catalog', 'Scaffold N50']]
#port_N50.columns = ['Catalog ID', 'N50']
pub_N50.columns = ['Catalog ID', 'N50']
...
df_N50.to_csv('Assembly_Scaffold_N50_comparisons.txt',sep='\t',index=False)

#contig/replicon count #compared by min
#port_contig= df_port_complete[['Base Catalog Number', 'Total Contigs']]
pub_contig = df_pub_scaffold_min[['ATCC Catalog', 'Scaffold Count']]
#port_contig.columns = ['Catalog ID', 'Contig Count']
pub_contig.columns = ['Catalog ID', 'Contig Count']
...
df_contig.to_csv('Assembly_Scaffold_Contig_comparisons.txt',sep='\t',index=False)

#GC Content #compared by mean
#port_gc= df_port_complete[['Base Catalog Number', 'GC Content']]
pub_gc = df_pub_scaffold_mean[['ATCC Catalog', 'GC Content']]
#port_gc.columns = ['Catalog ID', 'GC']
#port_gc['GC'] = port_gc['GC'] * 100
pub_gc.columns = ['Catalog ID', 'GC']
...
df_gc.to_csv('Assembly_Scaffold_GC_comparisons.txt',sep='\t',index=False)

#'contig' public assemblies
#genome length #compared by average length #comparison metric could be command line argument and consistent across all fields.
#port_length = df_port_complete[['Base Catalog Number', 'Total Length']]
pub_length = df_pub_contig_mean[['ATCC Catalog', 'Total Length']]
#port_length.columns = ['Catalog ID', 'Total Length']
pub_length.columns = ['Catalog ID', 'Total Length'] #need same column names to merge dataframes
df_len = pd.merge(port_length, pub_length, on=['Catalog ID'], suffixes=('_portal','_public'))
df_len['Length Ratio'] = df_len['Total Length_portal'] / df_len['Total Length_public']
df_len['Public Assembly Level'] = 'Contig'
df_len.to_csv('Assembly_Contig_length_comparisons.txt',sep='\t',index=False)

#N50 #compared by max N50
#port_N50= df_port_complete[['Base Catalog Number', 'Filtered N50']]
pub_N50 = df_pub_contig_mean[['ATCC Catalog', 'Scaffold N50']]
#port_N50.columns = ['Catalog ID', 'N50']
pub_N50.columns = ['Catalog ID', 'N50']
...
df_N50.to_csv('Assembly_Contig_N50_comparisons.txt',sep='\t',index=False)

#contig/replicon count #compared by min
#port_contig= df_port_complete[['Base Catalog Number', 'Total Contigs']]
pub_contig = df_pub_contig_min[['ATCC Catalog', 'Scaffold Count']]
#port_contig.columns = ['Catalog ID', 'Contig Count']
pub_contig.columns = ['Catalog ID', 'Contig Count']
...
df_contig.to_csv('Assembly_Contig_Contig_comparisons.txt',sep='\t',index=False)

#GC Content #compared by mean
#port_gc= df_port_complete[['Base Catalog Number', 'GC Content']]
pub_gc = df_pub_contig_mean[['ATCC Catalog', 'GC Content']]
#port_gc.columns = ['Catalog ID', 'GC']
#port_gc['GC'] = port_gc['GC'] * 100
pub_gc.columns = ['Catalog ID', 'GC']
...
df_gc.to_csv('Assembly_Contig_GC_comparisons.txt',sep='\t',index=False)
